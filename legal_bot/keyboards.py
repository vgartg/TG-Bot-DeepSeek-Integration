...
def get_pending_receipt_control(paid_request_id, has_next, has_prev):
    ...
    keyboard = [[InlineKeyboardButton("✅ Пометить как готовый", callback_data=f'admin_mark_issued_{paid_request_id}')]]
    nav_buttons = []
    if has_prev:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "🔒 Политика конфиденциальности и обработки персональных данных", callback_data='show_privacy'
            )
        ],
        [InlineKeyboardButton("💸 Политика возврата денежных средств", callback_data='show_return_money')],
        [InlineKeyboardButton("⭐ Главное меню", callback_data='main_menu')],
